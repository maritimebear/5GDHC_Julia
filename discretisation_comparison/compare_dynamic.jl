...
dynamic_solver = de.Rodas5
time_interval = (0.0, 1 * 60 * 60.0)
saveinterval = 1.0
...
results_dict = Dict{Int,
                    NamedTuple{(:syms, :sol), Tuple{Vector{Symbol}, de.SciMLBase.ODESolution}}
                   }()
...
for (i, discn) in enumerate(discretisations)
    ...
    results_dict[i] = (syms=nd_fn.syms,
                       sol = DHG.Miscellaneous.solve_dynamic(nd_fn, initial_guess, params, dynamic_solver(), time_interval, saveinterval),
                      )
    println(" --- done")
end
